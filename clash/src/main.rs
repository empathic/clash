...
enum Commands {
    ...
    /// Migrate legacy Claude Code permissions to a policy.toml file
    Migrate {
        /// Print generated policy to stdout instead of writing to ~/.clash/policy.toml
        #[arg(long)]
        dry_run: bool,

        /// Override the default effect for unmatched requests (ask, forbid)
...
/// into policy statements, and writes a `policy.toml` file.
fn run_migrate(dry_run: bool, default_effect: &str) -> Result<()> {
    let default = match default_effect {
        ...
        other => anyhow::bail!(
            "invalid default effect '{}': expected ask, forbid, or permit",
            other
        ),
    };
    ...
    let doc = PolicyDocument {
        ...
        permissions: None, // We desugar into statements, not legacy format
        statements,
    };

    let toml_output =
        toml::to_string_pretty(&doc).context("failed to serialize policy document to TOML")?;

    // Add a header comment
    let output = format!(
        "# Policy generated by `clash migrate` from Claude Code settings.\n\
    ...
    if dry_run {
        ...
        if path.exists() {
            ...
            anyhow::bail!("policy.toml already exists at {}", path.display());
        }
        std::fs::create_dir_all(settings::ClashSettings::settings_dir())?;
        std::fs::write(&path, &output)?;
        println!("Wrote policy to {}", path.display());
        println!(
            "Migrated {} statement(s) from legacy Claude Code permissions.",
            doc.statements.len()
        );
    }
