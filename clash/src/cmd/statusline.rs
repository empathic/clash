...
//! policy decision counts and the current enforcement posture.

use anyhow::{Context, Result};
use clap::Subcommand;
use serde::Deserialize;

use crate::audit::{SessionStats, read_session_stats};
use crate::style;
...
/// Minimal structure we need from the Claude Code status line JSON on stdin.
#[derive(Deserialize)]
struct StdinPayload {
    session_id: String,
}
...
///
/// Reads the session_id from the JSON payload on stdin, loads the stats
/// sidecar, and prints a formatted scoreboard.
fn render(format: &str) -> Result<()> {
    ...
    let stats = read_session_stats(&payload.session_id);
...
fn format_stats(stats: &SessionStats, _format: &str) -> String {
    ...
    let hint = match &stats.last_deny_hint {
        Some(cmd) if stats.last_effect.as_deref() == Some("deny") => {
...
/// Map an effect string to the same colored symbol used in the counts.
fn effect_symbol(effect: &str) -> String {
    match effect {
        "allow" => style::green("✓"),
        "deny" => style::red("✗"),
        "ask" => style::yellow("?"),
...
/// Extract a brief, human-readable summary from the tool input JSON.
///
/// Returns `None` if the input is empty or unhelpful.
fn brief_summary(input: &str) -> Option<&str> {
    if input.is_empty() || input == "{}" || input == "null" {
        return None;
    }
    // Already truncated to ~60 chars by update_session_stats.
    Some(input)
}
