...
pub struct Shell<SE: extensions::ShellExtensions = extensions::DefaultShellExtensions> {
    ...
    /// Optional hook for intercepting external commands before execution.
    #[cfg_attr(feature = "serde", serde(skip))]
    external_command_hook: Option<ExternalCommandHook>,
}
...
impl<SE: extensions::ShellExtensions> Clone for Shell<SE> {
    fn clone(&self) -> Self {
        Self {
            ...
            external_command_hook: self.external_command_hook.clone(),
            depth: self.depth + 1,
        }
    }
}
...
impl<SE: extensions::ShellExtensions> Shell<SE> {
    ...
    pub(crate) fn new(options: CreateOptions<SE>) -> Result<Self, error::Error> {
        ...
        let mut shell = Self {
            ...
            external_command_hook: options.external_command_hook,
            ..Self::default()
        };
...
impl<SE: extensions::ShellExtensions> Shell<SE> {
    ...
    /// Returns the external command hook, if set.
    pub fn external_command_hook(&self) -> Option<&ExternalCommandHook> {
        self.external_command_hook.as_ref()
    }
}
