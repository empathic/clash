...
pub(crate) fn execute_external_command(
    ...
    // Apply external command hook if present. The hook can replace the
    // executable path and arguments (e.g. to wrap commands in a sandbox).
    ...
    let (effective_exe, hooked_args) = if let Some(hook) = context.shell.external_command_hook() {
        if let Some((new_exe, new_args)) = hook(executable_path, &cmd_arg_strings) {
    ...
    let mut cmd = if let Some(ref new_args) = hooked_args {
        compose_std_command(
            &context,
            &effective_exe,
            &effective_exe,
