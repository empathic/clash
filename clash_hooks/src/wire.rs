...
pub(crate) fn parse_payload(
    ...
    let val = serde_json::Value::Object(extra.clone());
    match event_name {
        "PreToolUse" | "PostToolUse" | "PostToolUseFailure" | "PermissionRequest" => {
            serde_json::from_value(val)
                .map(EventPayload::Tool)
                .unwrap_or(EventPayload::Unknown(extra))
        }
        "SessionStart" => serde_json::from_value(val)
            .map(EventPayload::SessionStart)
            .unwrap_or(EventPayload::Unknown(extra)),
        "SessionEnd" => serde_json::from_value(val)
            .map(EventPayload::SessionEnd)
            .unwrap_or(EventPayload::Unknown(extra)),
        "UserPromptSubmit" => serde_json::from_value(val)
            .map(EventPayload::UserPromptSubmit)
            .unwrap_or(EventPayload::Unknown(extra)),
        "Stop" => serde_json::from_value(val)
            .map(EventPayload::Stop)
            .unwrap_or(EventPayload::Unknown(extra)),
        "StopFailure" => serde_json::from_value(val)
            .map(EventPayload::StopFailure)
            .unwrap_or(EventPayload::Unknown(extra)),
        "SubagentStart" => serde_json::from_value(val)
            .map(EventPayload::SubagentStart)
            .unwrap_or(EventPayload::Unknown(extra)),
        "SubagentStop" => serde_json::from_value(val)
            .map(EventPayload::SubagentStop)
            .unwrap_or(EventPayload::Unknown(extra)),
        "TeammateIdle" => serde_json::from_value(val)
            .map(EventPayload::TeammateIdle)
            .unwrap_or(EventPayload::Unknown(extra)),
        "TaskCompleted" => serde_json::from_value(val)
            .map(EventPayload::TaskCompleted)
            .unwrap_or(EventPayload::Unknown(extra)),
        "InstructionsLoaded" => serde_json::from_value(val)
            .map(EventPayload::InstructionsLoaded)
            .unwrap_or(EventPayload::Unknown(extra)),
        "ConfigChange" => serde_json::from_value(val)
            .map(EventPayload::ConfigChange)
            .unwrap_or(EventPayload::Unknown(extra)),
        "PreCompact" => serde_json::from_value(val)
            .map(EventPayload::PreCompact)
            .unwrap_or(EventPayload::Unknown(extra)),
        "PostCompact" => serde_json::from_value(val)
            .map(EventPayload::PostCompact)
            .unwrap_or(EventPayload::Unknown(extra)),
        "Notification" => serde_json::from_value(val)
            .map(EventPayload::Notification)
            .unwrap_or(EventPayload::Unknown(extra)),
        "Elicitation" => serde_json::from_value(val)
            .map(EventPayload::Elicitation)
            .unwrap_or(EventPayload::Unknown(extra)),
        "ElicitationResult" => serde_json::from_value(val)
            .map(EventPayload::ElicitationResult)
            .unwrap_or(EventPayload::Unknown(extra)),
        "WorktreeCreate" => serde_json::from_value(val)
            .map(EventPayload::WorktreeCreate)
            .unwrap_or(EventPayload::Unknown(extra)),
        "WorktreeRemove" => serde_json::from_value(val)
            .map(EventPayload::WorktreeRemove)
            .unwrap_or(EventPayload::Unknown(extra)),
...
/// Raw JSON object received from Claude Code on stdin (legacy flat struct).
///
/// This type is preserved for backward compatibility with code that accesses
/// fields directly. New code should use the typed event API instead.
#[derive(Debug, Clone, Deserialize)]
pub struct RawInput {
    pub session_id: String,
    pub transcript_path: String,
