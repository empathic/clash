...
pub fn install(dry_run: bool) -> Result<String> {
    ...
    let existing: Value = if path.exists() {
        serde_json::from_str(&std::fs::read_to_string(&path)?).context("parse zed settings.json")?
    } else {
        json!({})
    };
...
fn merge(mut existing: Value) -> Value {
    let obj = existing
        .as_object_mut()
        .expect("zed settings root is an object");
...
mod tests {
    ...
    fn merges_into_empty() {
        ...
        assert_eq!(
            merged["languages"]["Starlark"]["language_servers"][0],
            "clash-lsp"
        );
    }
