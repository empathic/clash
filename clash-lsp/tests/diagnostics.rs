...
async fn completion_returns_clash_builtins() {
    ...
    .await
    .expect("timed out waiting for completion response");
    ...
    let items = if resp["result"].is_array() {
        ...
        resp["result"]["items"]
            .as_array()
            .expect("completion result missing items")
            .clone()
    };
    let labels: std::collections::HashSet<String> = items
        .iter()
        .filter_map(|i| i.get("label").and_then(|l| l.as_str()).map(String::from))
        .collect();
    assert!(labels.contains("policy"), "missing 'policy' in {labels:?}");
    assert!(
        labels.contains("sandbox"),
        "missing 'sandbox' in {labels:?}"
    );
    assert!(
        labels.contains("settings"),
        "missing 'settings' in {labels:?}"
    );
