...
## Running clash
* `clash-cli` is an installed binary on the user's PATH. ALWAYS run it directly as `clash-cli` (e.g., `clash-cli status`, `clash-cli policy list`).
* NEVER use `cargo run --bin clash-cli` to run clash. That is for building/testing the crate, not for invoking the tool.
* Skills reference `clash-cli` commands — execute them exactly as written.
