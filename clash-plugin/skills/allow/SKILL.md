...
1. **Determine the rule** from the conversation context. Prefer broad patterns over narrow ones:
   ...
   - **S-expression rules** for specific patterns:
     - Exec: `clash-cli policy allow '(exec "git" *)'` — allow all git commands
     - Exec with subcommand: `clash-cli policy allow '(exec "git" "push" *)'` — allow git push
     - Fs: `clash-cli policy allow '(fs read (subpath (env PWD)))'` — allow reads under cwd
     - Fs with path: `clash-cli policy allow '(fs (subpath "/tmp"))'` — allow all fs ops under /tmp
     - Fs with env: `clash-cli policy allow '(fs write (subpath (env HOME)))'` — allow writes under home
     - Net: `clash-cli policy allow '(net "github.com")'` — allow network access to github.com
   - If the request involves a **directory path** or filesystem access (e.g., "allow access to ~/Library/Caches"):
     ...
     - Example: `clash-cli policy allow '(fs (subpath "/Users/me/Library/Caches"))'`
     - Use `(env HOME)` or `(env PWD)` for dynamic paths: `clash-cli policy allow '(fs (subpath (join (env HOME) "/Library/Caches")))'`
     - Optionally scope to an operation: `read`, `write`, `create`, `delete`, or combine with `(or read write)`
   - When unsure about the exact syntax, run `clash-cli policy allow --help` to check available options.
...
3. **Dry-run first** to preview the change:
   ...
   clash-cli policy allow '(fs write (subpath (env HOME)))' --dry-run
   ```
