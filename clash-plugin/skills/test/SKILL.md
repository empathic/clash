...
```bash
$PLUGIN_DIR/bin/clash explain bash "rm -rf /" --json
$PLUGIN_DIR/bin/clash explain read "/etc/passwd" --json
$PLUGIN_DIR/bin/clash explain bash "git push origin main" --json
$PLUGIN_DIR/bin/clash explain write "~/.ssh/config" --json
```
