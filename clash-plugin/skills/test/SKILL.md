...
```bash
$CLASH_BIN policy explain bash "rm -rf /" --json
$CLASH_BIN policy explain read "/etc/passwd" --json
$CLASH_BIN policy explain bash "git push origin main" --json
$CLASH_BIN policy explain write "~/.ssh/config" --json
```
